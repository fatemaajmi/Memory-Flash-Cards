...
let lifeCount = 3;
let interval; // Declare interval outside the countdown function
let cardsList = [
...
console.log('life decremented');
 updateLifeUI(); // Update the UI to reflect the new life count
...
countdownElement.innerHTML = 'Gave Over! No lifes left.. Try again';
   // Add any additional logic here, such as resetting the game or showing a replay button
}
flips = 0;
}
...
let updateFlips = (img) => {
 // Find the index of the image in cardsList
let imageName =
...
if (index !== -1) {
   // Increment the flips count for the image
...
if (cardsList[index].flips > 2) {
    decrementLife();
     cardsList[index].flips = 0; //reset flips for current image
}
} else {
